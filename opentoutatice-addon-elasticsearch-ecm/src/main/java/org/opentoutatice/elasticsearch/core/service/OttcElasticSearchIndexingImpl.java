...
public class OttcElasticSearchIndexingImpl /* extends ElasticSearchIndexingImpl */ implements OttcElasticSearchIndexing {
    ...
    public OttcElasticSearchIndexingImpl(OttcElasticSearchAdminImpl esa) {
        ...
        this.indexTimer = registry.timer(MetricRegistry.name("nuxeo", "elasticsearch", "service", "index"));
        this.deleteTimer = registry.timer(MetricRegistry.name("nuxeo", "elasticsearch", "service", "delete"));
        this.bulkIndexTimer = registry.timer(MetricRegistry.name("nuxeo", "elasticsearch", "service", "bulkIndex"));
     ...
     * {@inheritDoc}
     *
     * @throws ReIndexingStatusException
    ...
    public void indexNonRecursive(List<IndexingCommand> cmds) throws ClientException {
        ...
        Context stopWatch = this.bulkIndexTimer.time();
        try {
    ...
    void processBulkDeleteCommands(List<IndexingCommand> cmds) {
        ...
        for (IndexingCommand cmd : cmds) {
            if (cmd.getType() == Type.DELETE) {
                Context stopWatch = this.deleteTimer.time();
                try {
    ...
    void processBulkIndexCommands(List<IndexingCommand> cmds) throws ClientException {
        ...
        if (bulkRequest.numberOfActions() > 0) {
            if (log.isDebugEnabled()) {
                log.debug(String.format("Index %d docs in bulk request: curl -XPOST 'http://localhost:9200/_bulk' -d '%s'", bulkRequest.numberOfActions(),
    ...
    void processIndexCommand(IndexingCommand cmd) {
        ...
        try {
            ...
            log.info("Ignore indexing of doc " + cmd.getTargetDocumentId() + " a more recent version has already been indexed: " + e.getMessage());
        }
    }
    ...
    void processDeleteCommandRecursive(IndexingCommand cmd) {
        ...
        DeleteByQueryRequestBuilder deleteRequest = this.esa.getClient().prepareDeleteByQuery(indexName).setTypes(DOC_TYPE).setQuery(query);
        if (log.isDebugEnabled()) {
            log.debug(
                    String.format("Delete byQuery request: curl -XDELETE 'http://localhost:9200/%s/%s/_query' -d '%s'", indexName, DOC_TYPE, query.toString()));
        }
        ...
        for (IndexDeleteByQueryResponse response : responses) {
            ...
            if (response.getFailedShards() > 0) {
                log.error(String.format("Delete byQuery fails on shard: %d out of %d", response.getFailedShards(), response.getTotalShards()));
            }
        }
    }
    ...
    String getPathOfDocFromEs(String repository, String docId) {
        ...
        GetRequestBuilder getRequest = this.esa.getClient().prepareGet(indexName, DOC_TYPE, docId).setFields(PATH_FIELD);
        if (log.isDebugEnabled()) {
            log.debug(String.format("Get path of doc: curl -XGET 'http://localhost:9200/%s/%s/%s?fields=%s'", indexName, DOC_TYPE, docId, PATH_FIELD));
        }
